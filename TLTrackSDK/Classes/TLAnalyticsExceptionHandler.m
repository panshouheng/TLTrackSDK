...
- (instancetype)init {
    ...
    if (self) {
        ...
        NSSetUncaughtExceptionHandler(&TinecoLifeData_uncaught_exception_handler);
        ...
        // 设置信号集回调处理函数
        sig.sa_sigaction = &TinecoLifeData_signal_exception_handler;
        // 定义需要采集的信号类型
...
static void TinecoLifeData_signal_exception_handler(int sig, struct __siginfo *info, void *context) {
...
- (void)trackAppCrashedWithException:(NSException *)exception {
    ...
    [defaults setObject:exceptionInfo forKey:@"TinecoLifeData_app_crashed_reason"];
