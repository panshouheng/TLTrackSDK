...
static NSString * const TinecoAnalyticsVersion = @"1.2.2";
...
- (void)trackAppClickWithView:(UIView *)view properties:(nullable NSDictionary<NSString *, id> *)properties {
    NSMutableDictionary *eventProperties = [NSMutableDictionary dictionary];
    // 获取控件类型
    eventProperties[@"element_type"] = view.TinecoLifeData_elementType;
    // 获取控件显示文本
    eventProperties[@"element_content"] = view.TinecoLifeData_elementContent;
...
- (void)trackAppClickWithTableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath properties:(nullable NSDictionary<NSString *, id> *)properties {
    ...
    eventProperties[@"element_content"] = cell.TinecoLifeData_elementContent;
...
*/
static inline void TinecoLifeData_method_exchange(const char *className, const char *methodName1, const char *methodName2, IMP method2IMP) {
    // 通过类名获取类
...
- (void)enableTrackReactNativeEvent {
    TinecoLifeData_method_exchange("RCTUIManager", "setJSResponder:blockNativeResponder:", "TinecoLifeData_setJSResponder:blockNativeResponder:", (IMP)TinecoLifeData_setJSResponder);
}

static void TinecoLifeData_setJSResponder(id obj, SEL cmd, NSNumber *reactTag, BOOL blockNativeResponder) {
    // 先执行原来的方法
    SEL oriSel = sel_getUid("TinecoLifeData_setJSResponder:blockNativeResponder:");
    // 获取原始方法的实现函数指针
